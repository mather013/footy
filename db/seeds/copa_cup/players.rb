Player.delete_all

#argentina
Player.create(id: 1,   team_id: 1, squad_number: 20, forename: 'S',    surname: 'Aguero')
Player.create(id: 2,   team_id: 1, squad_number: 9,  forename: 'G',    surname: 'Higuain')
Player.create(id: 3,   team_id: 1, squad_number: 7,  forename: 'A di', surname: 'Maria')
Player.create(id: 4,   team_id: 1, squad_number: 10, forename: 'L',    surname: 'Messi')
Player.create(id: 5,   team_id: 1, squad_number: 11, forename: 'C',    surname: 'Tevez')

#bolivia - need numbers
Player.create(id: 21,  team_id: 2, squad_number: 7, forename: 'J',    surname: 'Arce')
Player.create(id: 22,  team_id: 2, squad_number: 0, forename: 'R',    surname: 'Pedriel')
Player.create(id: 23,  team_id: 2, squad_number: 9, forename: 'M',    surname: 'Moreno')
Player.create(id: 24,  team_id: 2, squad_number: 0, forename: 'P',    surname: 'Escobar')
Player.create(id: 25,  team_id: 2, squad_number: 0, forename: 'J',    surname: 'Campos')

#brazil
Player.create(id: 41,  team_id: 3, squad_number: 10, forename: nil, surname: 'Neymar Jr')
Player.create(id: 42,  team_id: 3, squad_number: 7,  forename: nil, surname: 'Robinho')
Player.create(id: 43,  team_id: 3, squad_number: 19, forename: nil, surname: 'Willian')
Player.create(id: 44,  team_id: 3, squad_number: 21, forename: 'P', surname: 'Coutinho')
Player.create(id: 45,  team_id: 3, squad_number: 9,  forename: 'D', surname: 'Tardelli')

#chile
Player.create(id: 61,  team_id: 4, squad_number: 7,  forename: 'A', surname: 'Sanchez')
Player.create(id: 62,  team_id: 4, squad_number: 8,  forename: 'A', surname: 'Vidal')
Player.create(id: 63,  team_id: 4, squad_number: 11, forename: 'E', surname: 'Vargas')
Player.create(id: 64,  team_id: 4, squad_number: 14, forename: 'M', surname: 'Fernandez')
Player.create(id: 65,  team_id: 4, squad_number: 9,  forename: 'M', surname: 'Pinilla')

#colombia - need numbers
Player.create(id: 81,  team_id: 5, squad_number: 17, forename: 'C', surname: 'Bacca')
Player.create(id: 82,  team_id: 5, squad_number: 19, forename: 'T', surname: 'Gutierrez')
Player.create(id: 83,  team_id: 5, squad_number: 21, forename: 'J', surname: 'Martinez')
Player.create(id: 84,  team_id: 5, squad_number: 10, forename: 'J', surname: 'Rodriguez')
Player.create(id: 85,  team_id: 5, squad_number: 9,  forename: 'R', surname: 'Falcao')

#ecuador - need numbers
Player.create(id: 101, team_id: 6, squad_number: 16, forename: 'A', surname: 'Valencia')
Player.create(id: 102, team_id: 6, squad_number: 0, forename: 'J', surname: 'Ayovi')
Player.create(id: 103, team_id: 6, squad_number: 0, forename: 'E', surname: 'Valencia')
Player.create(id: 104, team_id: 6, squad_number: 0, forename: 'J', surname: 'Montero')
Player.create(id: 105, team_id: 6, squad_number: 0, forename: 'F', surname: 'Martinez')

#jamaica - need numbers
Player.create(id: 121, team_id: 7, squad_number: 0, forename: 'D', surname: 'Mattocks')
Player.create(id: 122, team_id: 7, squad_number: 0, forename: 'D', surname: 'Brown')
Player.create(id: 123, team_id: 7, squad_number: 0, forename: 'R', surname: 'Austin')
Player.create(id: 124, team_id: 7, squad_number: 0, forename: 'S', surname: 'Dawkins')
Player.create(id: 125, team_id: 7, squad_number: 0, forename: 'G', surname: 'McCleary')

#mexico
Player.create(id: 141, team_id: 8, squad_number: 4,  forename: 'R', surname: 'Marquez')
Player.create(id: 142, team_id: 8, squad_number: 10, forename: 'L', surname: 'Montes')
Player.create(id: 143, team_id: 8, squad_number: 8,  forename: 'M', surname: 'Fabian')
Player.create(id: 144, team_id: 8, squad_number: 19, forename: 'V', surname: 'Vuoso')
Player.create(id: 145, team_id: 8, squad_number: 9,  forename: 'R', surname: 'Jimenez')

#paraguay
Player.create(id: 161, team_id: 9, squad_number: 9,  forename: 'R santa', surname: 'Cruz')
Player.create(id: 162, team_id: 9, squad_number: 18, forename: 'N', surname: 'Valdez')
Player.create(id: 163, team_id: 9, squad_number: 8,  forename: 'L', surname: 'Barrios')
Player.create(id: 164, team_id: 9, squad_number: 11, forename: 'E', surname: 'Benitez')
Player.create(id: 165, team_id: 9, squad_number: 23, forename: 'R', surname: 'Ortiz')

#peru
Player.create(id: 181, team_id: 10, squad_number: 22, forename: 'C', surname: 'Ascues')
Player.create(id: 182, team_id: 10, squad_number: 10, forename: 'J', surname: 'Farfan')
Player.create(id: 183, team_id: 10, squad_number: 18, forename: 'A', surname: 'Carrillo')
Player.create(id: 184, team_id: 10, squad_number: 20, forename: 'C', surname: 'Cueva')
Player.create(id: 185, team_id: 10, squad_number: 14, forename: 'C', surname: 'Pizarro')

#uruguay
Player.create(id: 201, team_id: 11, squad_number: 21, forename: 'E', surname: 'Cavani')
Player.create(id: 202, team_id: 11, squad_number: 8,  forename: 'A', surname: 'Hernandez')
Player.create(id: 203, team_id: 11, squad_number: 11, forename: 'C', surname: 'Stuani')
Player.create(id: 204, team_id: 11, squad_number: 7,  forename: 'C', surname: 'Rodriguez')
Player.create(id: 205, team_id: 11, squad_number: 10, forename: 'G de', surname: 'Arrascaeta')

#venezuela - need numbers
Player.create(id: 221, team_id: 12, squad_number: 0, forename: nil, surname: 'Miku')
Player.create(id: 222, team_id: 12, squad_number: 0, forename: 'S', surname: 'Rondon')
Player.create(id: 223, team_id: 12, squad_number: 0, forename: 'J', surname: 'Arango')
Player.create(id: 224, team_id: 12, squad_number: 0, forename: 'J', surname: 'Martinez')
Player.create(id: 225, team_id: 12, squad_number: 0, forename: 'C', surname: 'Gonzalez')
