    ...
    class << self
      def perform
        ...
        bets = [{ user_id: User.find_by_username('markm').id,  player_names: ['K Benzema', 'L Messi',     'Neymar Jr',   'J Rodriguez',  'L Suarez'   ] },
                ...
                { user_id: User.find_by_username('samt').id,   player_names: ['S Aguero',  'K Benzema',   'Fred',        'L Messi',      'C Ronaldo'  ] },
                ...
                { user_id: User.find_by_username('genec').id,  player_names: ['Fred',      'G Higuain',   'Neymar Jr',   'R van Persie', 'J Rodriguez'] },
