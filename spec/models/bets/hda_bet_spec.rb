...
describe Bets::HdaBet do
  ...
  describe 'validation' do
    ...
    context 'when user has already created a bet for this fixture' do
      it 'is an invalid bet' do
        Bets::HdaBet.create(fixture_id: 1, user_id: 1, value: 'H')
        Bets::HdaBet.new(fixture_id: 1, user_id: 1, value: 'H').should_not be_valid
      end
    end
    ...
    context 'when user has not yet created a bet for this fixture' do
      ...
      it 'is a valid bet' do
        Bets::HdaBet.create(fixture_id: 1, user_id: 1, value: 'H')
        Bets::HdaBet.new(fixture_id: 2, user_id: 1, value: 'H').should be_valid
      end
    end
    ...
    describe 'check_permitted' do
      ...
      context 'when creating bet' do
        ...
        context 'when fixture has not yet kickoff' do
          ...
          it 'allows the bet to be created' do
            bet = user.hda_bets.create(fixture_id: fixture_1.id, value: 'H')
            expect(Bets::HdaBet.count).to eq 1
          end
        end
        ...
        context 'when fixture has kicked off' do
          ...
          it 'does not allow the bet to be created' do
            ...
            expect(Bets::HdaBet.count).to eq 0
          end
        end
      end
      ...
      context 'when updating bet' do
        let(:kickoff) { 1.day.from_now }
        let!(:bet) { user.hda_bets.create(fixture_id: fixture_1.id, value: 'H') }
  ...
  describe 'scopes' do
    ...
    let!(:bet_one) { Bets::HdaBet.create(user_id: 1, fixture_id: fixture_1.id, value: 'H') }
    let!(:bet_two) { Bets::HdaBet.create(user_id: 2, fixture_id: fixture_2.id, value: 'H') }

    it 'points by descending value' do
      Bets::HdaBet.bets_for_user_and_fixtures(user_one, [1, 2]).should eq [bet_one]
    end

    it 'creates the expected sql for returning bets for user and fixtures' do
      expect(Bets::HdaBet.bets_for_user_and_fixtures(user_one, [1, 2, 3]).to_sql.should == "SELECT \"hda_bets\".* FROM \"hda_bets\"  WHERE (user_id = 1 and fixture_id in (1,2,3))")
    end
  end
  ...
  describe 'instance methods' do
    ...
    let!(:bet) { Bets::HdaBet.create(fixture_id: 1, user_id: 1, value: bet_value) }
