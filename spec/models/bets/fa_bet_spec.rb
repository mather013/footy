  ...
  describe 'validation' do
    ...
    context 'when user has already created an fa_bet with this player' do
      it 'is an invalid bet' do
        Bets::FaBet.create(player_id: 1, user_id: user.id)
        Bets::FaBet.new(player_id: 1, user_id: user.id).should_not be_valid
      end
    end
    ...
    context 'when user has not yet created a fa_bet with this player' do
      it 'is a valid bet' do
        Bets::FaBet.create(player_id: 1, user_id: user.id)
        Bets::FaBet.new(player_id: 2, user_id: user.id).should be_valid
      end
    end
  end
  ...
  describe 'instance methods' do
    ...
    describe 'check_combination_unique?' do
      ...
      context 'when creating 5 alive bet' do
        before :each do
          user_one_selections.each { |player_id| Bets::FaBet.create(user_id: user_one.id, player_id: player_id) }
          user_two_selections.each { |player_id| Bets::FaBet.create(user_id: user_two.id, player_id: player_id) }

          @bet = Bets::FaBet.create(user_id: user_one.id, player_id: 5)
        end
      ...
      context 'when updating 5 alive bet' do
        before :each do
          ...
          @bet = Bets::FaBet.create(user_id: user_one.id, player_id: 7)
