...
describe TeamStatsLeague do
  ...
  describe 'results and league_stats' do
    ...
    it 'returns teams stats ordered by kickoff' do
      allow(Week).to receive(:league_weeks).and_return([week_1, week_2, week_3])

      expect(TeamStatsLeague.new(liverpool).perform).to eq({pld: 3, pts: 4, gf: 5, ga: 4, gd: 1,  form: 'LDW', results: %w'L D W'})
      expect(TeamStatsLeague.new(newcastle).perform).to eq({pld: 3, pts: 4, gf: 4, ga: 5, gd: -1, form: 'WDL', results: %w'W D L'})
      expect(TeamStatsLeague.new(everton).perform).to eq({pld: 0, pts: 0, gf: 0, ga: 0, gd: 0, form: '', results: []})
    end
